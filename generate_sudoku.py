...
def random_mrows(game, row):
    ...
    # add all the blocks from the 3rd block of the upper row to the 
    # second block of the current row, if not already in the first block
    ...
    # if there are still some moves needed for the second block (i.e if there are still zeros in second_block),
    # generate legal random moves until second_block is complete
    second_block_col = 3
    ...
    final_row = [inner for outer in final_row for inner in outer] # turn list of lists to just 1 list

    # if there are moves still needed for the row, call the switch_moves method to potentially fix the row
    if len(upper_row) == 1:
        for column in range(9):
...
def switch_moves(game, row, column, move):
    ...
    for check_column in range(1, column + 1): 
        check_move = game[row][column - check_column] # iterate through the row backwards, starting from the column missing

        # switch the numbers from the column checked, and the missing number
        temp = game[row][column]
        game[row][column] = 0
        game[row][column - check_column] = 0

        if legal_move(game, row, column - check_column, move) \
        and legal_move(game, row, column, check_move): # if both moves switched are legal:
...
def gen_rand():
    # generates a random finished game of sudoku
...
def print_game(game):
    # prints the game to look like an actual board
    for i in game:
        print(i)
